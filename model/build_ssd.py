...
def build_ssd(phase, size=300, cfg=None):
    ...
    if size != 300 and size!=600 and size!=800:
        ...
        return
    print(phase)
    #model,feature_map,feature_channel = get_pretrainedmodel(model_name,feature_map_out = [5,6,7,8])
...
class SSD(nn.Module):
    ...
    def forward(self, x, phase):
        ...
        for (x, l, c) in zip(sources, self.loc, self.conf):
            print(l(x).shape,l(x).permute(0,2,3,1).shape)
            print(c(x).shape,c(x).permute(0,2,3,1).shape)
...
def multibox(resnet50, extra_layers, cfg, num_classes):
    loc_layers = []
    conf_layers = []
    #inceptionv4_out_channel = [1024,1536]
    resnet_out_channel = [512, 1024]
    for k, v in enumerate(resnet_out_channel):
