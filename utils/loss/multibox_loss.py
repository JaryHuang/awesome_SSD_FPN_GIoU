...
from ..box import bbox_overlaps_giou,decode
...
class CDGiouLoss(nn.Module):
    ...
    """
    def __init__(self,pred_mode = 'Center',size_sum=True,variances=None,method = "GIoU"):
        super(CDGiouLoss, self).__init__()
    ...
    def forward(self, loc_p, loc_t,prior_data):
        ...
        ious =1.0 - bbox_overlaps_giou(decoded_boxes,loc_t,self.method)
...
class MultiBoxLoss(nn.Module):
    ...
    def __init__(self, cfg, overlap_thresh, prior_for_matching,
        ...
        self.ious = CDGiouLoss(pred_mode = 'Center',size_sum=True,variances=self.variance,method = self.loss_r)
        
        if self.loss_r not in ['SmoothL1',"GIoU","CIoU","DIoU"]:
            assert Exception("THe loss_r is Error, loss name must be SmoothL1 or Giou")
        elif self.loss_c not in ["CrossEntropy","FocalLoss"]:
    ...
    def forward(self, predictions, targets):
        ...
        if self.loss_r == 'SmoothL1':
            loss_l = F.smooth_l1_loss(loc_p, loc_t, reduction='sum')
        elif self.loss_r in ["GIoU","CIoU","DIoU"]:
